...
import com.astocoding.unsafe.UnsafeVisitable;
import org.openjdk.jcstress.infra.results.I_Result;

public final class UnsafeVisitable_jcstress extends Runner<I_Result> {

    volatile WorkerSync workerSync;
    UnsafeVisitable[] gs;
    I_Result[] gr;

    public UnsafeVisitable_jcstress(ForkedTestConfig config) {
        super(config);
    }
    ...
    private void sanityCheck_API(Counter<I_Result> counter) throws Throwable {
        final UnsafeVisitable s = new UnsafeVisitable();
    ...
    private void sanityCheck_Footprints(Counter<I_Result> counter) throws Throwable {
        config.adjustStrideCount(new FootprintEstimator() {
          public void runWith(int size, long[] cnts) {
            ...
            UnsafeVisitable[] ls = new UnsafeVisitable[size];
            I_Result[] lr = new I_Result[size];
            for (int c = 0; c < size; c++) {
                UnsafeVisitable s = new UnsafeVisitable();
    ...
    public ArrayList<CounterThread<I_Result>> internalRun() {
        ...
        gs = new UnsafeVisitable[len];
        gr = new I_Result[len];
        for (int c = 0; c < len; c++) {
            gs[c] = new UnsafeVisitable();
            gr[c] = new I_Result();
        }
    ...
    private void jcstress_consume(Counter<I_Result> cnt, int a) {
        ...
        for (int c = left; c < right; c++) {
            I_Result r = lr[c];
            UnsafeVisitable s = ls[c];
            ls[c] = new UnsafeVisitable();
            cnt.record(r);
            r.r1 = 0;
        }
    }
    ...
    private void run_method(UnsafeVisitable[] gs, I_Result[] gr, int start, int end) {
        UnsafeVisitable[] ls = gs;
        I_Result[] lr = gr;
    ...
    private Counter<I_Result> task_visitable() {
        int len = config.strideSize * config.strideCount;
        int stride = config.strideSize;
        Counter<I_Result> counter = new Counter<>();
        ...
        while (true) {
            ...
            int check = 0;
            for (int start = 0; start < len; start += stride) {
                run_visitable(gs, gr, start, start + stride);
                check += 2;
                sync.awaitCheckpoint(check);
            }
    ...
    private void run_visitable(UnsafeVisitable[] gs, I_Result[] gr, int start, int end) {
        UnsafeVisitable[] ls = gs;
        I_Result[] lr = gr;
        for (int c = start; c < end; c++) {
            UnsafeVisitable s = ls[c];
